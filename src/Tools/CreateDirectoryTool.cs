...
namespace AIDA
{
    public class CreateDirectoryTool : ExecutableFunction
    {
        ...
        public override Task<string> ExecuteAsync(JObject? arguments = null)
        {
            ...
            {
                return Task.FromResult("Directory at '" + path + "' already exists!");
            }
            ...
            try
            {
                ...
                return Task.FromResult("Directory '" + path + "' was successfully created.");
            }
            catch (Exception ex)
            {
                AnsiConsole.MarkupLine("[gray][italic]failed[/][/]");
                return Task.FromResult("Creation of directory failed. Exception message: " + ex.Message);
            }
        }
    }
}
